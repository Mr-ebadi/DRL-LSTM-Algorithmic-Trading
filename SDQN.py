...
class SDQN:
    ...
    def getNormalizationCoefficients(self, tradingEnv):
        ...
        sentiments = tradingData['Sentiment'].tolist()
        ...
        coefficients.append(coeffs)
        # 5. Sentiments => minimum and maximum values
        coeffs = (np.min(sentiments)/margin, np.max(sentiments)*margin)
        coefficients.append(coeffs)
    ...
    def processState(self, state, coefficients):
        ...
        sentiments = [state[4][i] for i in range(len(state[4]))]
        ...
        # 5. Sentiments => MinMax normalization
        sentiments = [sentiments[i] for i in range(1, len(sentiments))]
        if coefficients[4][0] != coefficients[4][1]:
            state[4] = [((x - coefficients[4][0])/(coefficients[4][1] - coefficients[4][0])) for x in sentiments]
        else:
            state[4] = [0 for x in sentiments]
    ...
    def training(self, trainingEnv, trainingParameters=[],
        ...
        if showPerformance:
            TrainTest = "Train"
            analyser = PerformanceEstimator(trainingEnv.data)
            analyser.displayPerformance('SDQN', TrainTest)
